<template>
	<table :class="['vuetable', css.tableClass]">
				...
				<template v-if="field.visible">
					...
					<template v-else>
						<th @click="orderBy(field, $event)"
							...
							{{ getTitle(field) }}&nbsp;
							<i v-if="isInCurrentSortGroup(field)" :class="sortIcon(field)"
							   :style="{opacity: sortIconOpacity(field)}"></i>
						</th>
					</template>
				</template>
		...
		<template v-for="(item, index) in tableData">
			<tr @dblclick="onRowDoubleClicked(item, $event)" @click="onRowClicked(item, $event)"
					...
					<template v-if="field.visible">
						<template v-if="isSpecialField(field.name)">
							...
							<td v-if="extractName(field.name) === '__component'"
								...
								<component :is="extractArgs(field.name)" :row-data="item"
								           :row-index="index"></component>
							</td>
...
<script>
	...
	//	import VtThumbnails from './VtThumbnails'
	Vue.use(VueResource)
...
//			'vt-thumbnails': VtThumbnails
		...
		props: {
			...
			css: {
				...
				default: function () {
					return {
						tableClass: 'table is-striped is-narrow',
